...
// Spotlight Fix: Force Precomposed Unicode for Spotlight
NSSet *_forcePrecomposedAppSet = [NSSet
    setWithArray:@[ @"com.apple.Spotlight", @"com.apple.systemuiserver" ]];

// Spotlight Fix: Check macOS version
BOOL isTargetMacOSVersion(void) {
  NSOperatingSystemVersion version =
      [[NSProcessInfo processInfo] operatingSystemVersion];
  return version.majorVersion >= 26;
}

// Spotlight Fix: Check if Spotlight is active using Accessibility API
...
// Spotlight Fix: Tiny delay helper for timing-sensitive Spotlight input
...
// Spotlight Fix: Replace text directly using Accessibility API (Primary Fix)
static BOOL ReplaceFocusedTextViaAX(NSInteger backspaceCount,
                                    NSString *insertText) {
  // ... [Setup AX SystemWide and FocusedElement] ...
  AXUIElementRef systemWide = AXUIElementCreateSystemWide();
  AXUIElementRef focusedElement = NULL;
  AXError error = AXUIElementCopyAttributeValue(
      systemWide, kAXFocusedUIElementAttribute, (CFTypeRef *)&focusedElement);
  ...
  error = AXUIElementCopyAttributeValue(focusedElement, kAXValueAttribute,
                                        &valueRef);
  if (error != kAXErrorSuccess) {
    CFRelease(focusedElement);
    return NO;
  }
  NSString *valueStr = (__bridge NSString *)valueRef;
  // ... [Get Selected Range] ...
  CFTypeRef rangeRef = NULL;
  error = AXUIElementCopyAttributeValue(
      focusedElement, kAXSelectedTextRangeAttribute, &rangeRef);
  if (error != kAXErrorSuccess) {
    if (valueRef)
      CFRelease(valueRef);
    CFRelease(focusedElement);
  ...
  AXValueGetValue((AXValueRef)rangeRef, kAXValueTypeCFRange, &selection);
  // ... [Calculate New String] ...
  NSInteger caretLocation = selection.location;
  NSInteger start = caretLocation - backspaceCount;
  if (start < 0)
    start = 0;
  NSString *newValue = [valueStr
      stringByReplacingCharactersInRange:NSMakeRange(start,
                                                     caretLocation - start)
                              withString:insertText];
  // ... [Set New Value] ...
  AXError setValueErr = AXUIElementSetAttributeValue(
      focusedElement, kAXValueAttribute, (__bridge CFTypeRef)newValue);
  // ... [Update Selection/Cursor] ...
  if (setValueErr == kAXErrorSuccess) {
    CFRange newSel = CFRangeMake(start + insertText.length, 0);
    AXValueRef newRange = AXValueCreate(kAXValueTypeCFRange, &newSel);
    AXUIElementSetAttributeValue(focusedElement, kAXSelectedTextRangeAttribute,
                                 newRange);
    if (newRange)
      CFRelease(newRange);
    return YES;
...
void SendNewCharString(const bool &dataFromMacro = false,
  ...
  // Spotlight Fix
  BOOL isSpotlight = isTargetMacOSVersion() && isSpotlightActive();
  if (isSpotlight) {
    // 1. Tiny delay BEFORE AX to allow event processing (fix 'too' -> 'o')
    SpotlightTinyDelay();

    NSString *insertStr = [NSString
        stringWithCharacters:_newCharString
                      length:(_willContinuteSending ? 16
                                                    : _newCharSize - offset)];
    NSInteger axBackspaceCount = (offset == 0) ? pData->backspaceCount : 0;
    if (ReplaceFocusedTextViaAX(axBackspaceCount, insertStr)) {
      return;
    }
    // 2. Fallback: If AX failed but we are in Spotlight (and thus skipped
    // backspaces in Callback),
    //    we must manually send them here.
    if (offset == 0 && pData->backspaceCount > 0) {
...
CGEventRef OpenKeyCallback(CGEventTapProxy proxy, CGEventType type,
  ...
  if (vOtherLanguage) {
    ...
    if (isource != NULL) {
      CFArrayRef languages = (CFArrayRef)TISGetInputSourceProperty(
          isource, kTISPropertyInputSourceLanguages);
  ...
  if (type == kCGEventKeyDown) {
    ...
    if (pData->code == vDoNothing) {    // do nothing
      ...
      // fix autocomplete
      // Spotlight Fix: Disable autofill fix (extra space) if Spotlight is
      // active
      if (vFixRecommendBrowser && pData->extCode != 4 &&
          !(isTargetMacOSVersion() && isSpotlightActive())) {
      ...
      if (pData->backspaceCount > 0 && pData->backspaceCount < MAX_BUFF) {
        // Spotlight Fix: skip backspace if handled by AX
        // Only apply if macOS >= 26 and Spotlight is active
        if (!(isTargetMacOSVersion() && isSpotlightActive())) {
          for (_i = 0; _i < pData->backspaceCount; _i++) {
            SendBackspace();
          }
        }
